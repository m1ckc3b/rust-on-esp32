...
fn main() -> Result<()> {
    ...
    let red_led = RefCell::new(PinDriver::output(peripherals.pins.gpio1).unwrap());
    let green_led = RefCell::new(PinDriver::output(peripherals.pins.gpio2).unwrap());

    // Setting leds
    red_led.borrow_mut().set_low()?;
    green_led.borrow_mut().set_low()?;
